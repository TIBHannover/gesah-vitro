...
@WebServlet(name = "SearchFacetsController", urlPatterns = { "/searchfacets" })
public class SearchFacetsController extends VitroAjaxController {
    ...
    protected void doRequest(VitroRequest vreq, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            SearchQuery query = getQuery(term, field);
            if (query == null) {
                log.debug("query for '" + term + "' is null.");
                setEmptySearchResults(response);
                return;
            }
    ...
    private SearchQuery getQuery(String term, String field) {
        ...
        query.setQuery("*:*");
        query.addFields(field, VitroSearchTermNames.URI);
        return query;
    }
